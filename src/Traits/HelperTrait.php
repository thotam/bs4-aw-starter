...
trait HelperTrait
{
     ...
     * @return void
     */
    protected function addPropertyArray($name, $after, $insert, $path, $space = '        ')
    {
        $contents = file_get_contents($path);

        $sub_contents = Str::before(Str::after($contents, $name . ' = ['), '];');

        if (!Str::contains($sub_contents, $insert)) {
            $replace_sub_contents = str_replace(
                $after,
                $after . PHP_EOL . $space . $insert,
                $sub_contents,
            );
